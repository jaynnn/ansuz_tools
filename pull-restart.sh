...
# Ansuz Tools 一键拉取代码 + 重启脚本（基于 PM2）
# 用法: ./pull-restart.sh
# 功能: 拉取最新代码 → 安装依赖 → 编译构建 → PM2 平滑重启（不影响其他 PM2 进程）
...
print_header "Ansuz Tools 一键拉取 + 重启"
...
# 步骤 6: PM2 平滑重启（仅针对 ansuz_tools，不影响其他进程）
print_header "步骤 6: PM2 平滑重启"
cd "$SCRIPT_DIR"

if pm2 describe ansuz_tools >/dev/null 2>&1; then
    print_message "检测到服务正在运行，正在平滑重启..." "$YELLOW"
    pm2 reload ecosystem.config.js --only ansuz_tools
...
echo "  pm2 reload ansuz_tools        - 零停机重启"
echo ""
